...
TEST_CASE("Bruneton atmosphere file rejects zero combined scattering coefficients")
{
    BrunetonAtmosphereData data = makeData();
    data.parameters.mieScattering[0] = 0.0f;

    std::ostringstream output(std::ios::binary);
    std::string error;
    CHECK_FALSE(SaveBrunetonAtmosphere(output, data, error));
    CHECK(error ==
