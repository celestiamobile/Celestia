...
{
    if (util::is_set(props.texUsage, TexUsage::BumpTexture))
    {
        unsigned int texCoordIndex = (!props.hasSharedTextureCoords() && util::is_set(props.texUsage, TexUsage::DiffuseTexture)) ? 1 : 0;
...
buildRingsVertexShader(const ShaderProperties& props, bool fisheyeEnabled)
{
    ...
    source += "position = v_Position.xyz * (ringRadius + ringWidth * in_TexCoord0.s);\n";
    if (props.hasEclipseShadows())
    {
        for (unsigned int i = 0; i < props.nLights; i++)
        {
...
CelestiaGLProgram::initSamplers()
{
    ...
    if (util::is_set(props.texUsage, TexUsage::BumpTexture))
    {
        if (GLint slot = glGetUniformLocation(program.getID(), "bumpTex"); slot != -1)
            glUniform1i(slot, nSamplers);
        nSamplers++;
    }
