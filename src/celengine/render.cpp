...
Renderer::Renderer() :
    ...
    windowHeight(0),
    fov(standardFOV),
    cosViewConeAngle(1.0 / std::sqrt(3.0)),
    screenDpi(96),
    corrFac(1.12f),
...
void Renderer::setProjectionMode(shared_ptr<celestia::engine::ProjectionMode> _projectionMode)
{
    ...
    shaderManager->setFisheyeEnabled(projectionMode->isFisheye());
    markSettingsChanged();
}
                             ...
                             bool special)
{
    ...
    Vector3f win;
    bool success = projectionMode->isFisheye() ? ProjectFisheye(pos, m_modelMatrix, m_projMatrix, view, win) : ProjectPerspective(pos, m_MVPMatrix, view, win);
    if (success)
    {
...
{
    return Quaterniond(m_cameraTransform) * m_cameraOrientation;
}
                    ...
                    const Selection& sel)
{
    ...
    m_cameraOrientation = observer.getOrientation();
