...
// See header. Direct CPU port of the precompute-
...
void RunJobs(Fn fn, int count, int num_threads) {
  ...
  int created = 0;
  try {
    for (; created < num_threads - 1; ++created)
      threads[created] = std::thread(worker);
  } catch (const std::system_error&) {
    // The calling thread completes any jobs not claimed by created workers.
  }
  worker();
...
PrecomputedTextures Precompute(const AtmosphereParameters& atm,
  ...
  const bool emulate_half = settings.emulate_half_precision;
  const int num_scattering_orders = settings.scattering_orders;
