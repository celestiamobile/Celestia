...
// brunetonprecompute.h — a standalone, CPU-only C++17 port of Eric Bruneton's
...
#ifndef BRUNETON_CPU_BRUNETONPRECOMPUTE_H_
#define BRUNETON_CPU_BRUNETONPRECOMPUTE_H_
...
constexpr int SCATTERING_TEXTURE_MU_SIZE = 256;
...
constexpr int SCATTERING_TEXTURE_HEIGHT = SCATTERING_TEXTURE_MU_SIZE;  // 256
...
// If true, Precompute rounds every 3D scattering texture store to IEEE half
// precision (round-to-nearest-even), emulating the RGBA16F storage the GPU uses
// for the scattering textures under use_half_precision. Transmittance and
// irradiance are always GL_RGBA32F on the GPU, so they are never rounded.
// Enable this to bit-match a half-precision baked LUT (e.g. the demo earth.atm).
extern bool g_emulate_half;
...
#endif  // BRUNETON_CPU_BRUNETONPRECOMPUTE_H_
